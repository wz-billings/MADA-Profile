...
Now let's do some models. This WOULD be a lot faster with a workflowset, but I ran into some problems that seemed quite time consuming to debug. So I'll just repeat a lot of code and have 1000 lines of code for this project. (Author's note: I actually tried using a workflowset, but got errors that just don't happen when I ran the exact same code outside of `workflow_map()`. I hope that `workflowsets` continues to mature, but I didn't have time to solve my issues for this assignment!)
...
Next up I'll try a random forest. I love these things. (Can you tell I'm running out of things to say for each model?) Note that I didn't even train the `trees` hyperparameter here. Once I watched a video where I think that Julia Silge said that 2000 trees usually works well (though I don't have the citation immediately, I'll link it if I find it and remember) so I just set that. I apologize to Julia if she didn't actually say that.
